...
# A crashed git process can leave .git/*.lock files behind, blocking all
# subsequent git operations. We remove them when no git process is active.
# Common lock files: index.lock (staging), HEAD.lock (commit), refs/*.lock (branch update).
GIT_LOCKS=( ".git/index.lock" ".git/HEAD.lock" )
for GIT_LOCK in "${GIT_LOCKS[@]}"; do
    if [ -f "$GIT_LOCK" ]; then
...
Co-Authored-By: Claude Sonnet 4.6 <noreply@anthropic.com>"; then
